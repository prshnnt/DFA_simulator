...
# Configuration
SCREEN_WIDTH, SCREEN_HEIGHT, FPS = 800, 650, 60
STATE_RADIUS = 35

# Colors
COLORS = {
    'bg': (25, 25, 35), 'state': (50, 50, 65), 'border': (150, 150, 170),
    'text': (255, 255, 255), 'transition': (100, 100, 120), 'highlight': (255, 220, 50),
    'token': (100, 180, 255), 'active': (50, 150, 255), 'accepted': (50, 220, 100),
    'rejected': (255, 80, 80), 'input_bg': (40, 40, 55), 'button': (60, 60, 80),
    'button_hover': (80, 80, 100), 'disabled': (40, 40, 50),
}
...
class InputBox:
    ...
    def handle_text(self, char: str) -> bool:
        if self.active and char.lower() in self.alphabet:
            self.text += char
            return True
        return False
...
class Animator:
    def __init__(self):
        ...
        self.token_pos = (0.0, 0.0)
        self.token_start = (0.0, 0.0)
        self.token_end = (0.0, 0.0)
    ...
    def start_pause(self, duration: int = 200) -> None:
        self.pause_elapsed = 0
...
class Renderer:
    def __init__(self, screen: pygame.Surface):
        self.screen = screen
        self.fonts = (pygame.font.Font(None, 36), pygame.font.Font(None, 24), pygame.font.Font(None, 18))
    ...
    def draw_state(self, state: str, is_final: bool, is_current: bool, is_start: bool,
                   pulse_radius: Optional[float], status_color: Optional[Tuple[int, int, int]]) -> None:
        ...
        if is_start:
            self._draw_arrow((x - STATE_RADIUS * 2.5, y), (x - STATE_RADIUS, y), COLORS['border'])

    def draw_transitions(self, dfa: DFA, highlighted: Optional[Tuple[str, str, str]]) -> None:
        for (frm, sym), to in dfa.transitions.items():
            is_hl = highlighted and highlighted[0] == frm and highlighted[2] == to
    ...
    def _draw_curved_arrow(self, start: Tuple[float, float], end: Tuple[float, float],
        ...
        cx, cy = mx - (ey - sy) / dist * 30, my + (ex - sx) / dist * 30
    ...
    def _draw_arrowhead(self, start: Tuple[float, float], end: Tuple[float, float],
        ...
        al = 12
        aa = math.pi / 6
        p1 = (ex - al * math.cos(angle - aa), ey - al * math.sin(angle - aa))
        p2 = (ex - al * math.cos(angle + aa), ey - al * math.sin(angle + aa))
    ...
    def draw_input_box(self, rect: pygame.Rect, text: str, active: bool) -> None:
        ...
        pygame.draw.rect(self.screen, COLORS['active'] if active else COLORS['border'], rect, 2)
    ...
    def draw_symbol(self, sym: str, idx: int, total: int) -> None:
        txt = self.fonts[1].render(f"Symbol: '{sym}' ({idx + 1}/{total})", True, COLORS['text'])
...
def calc_positions(states: List[str], cx: float, cy: float, r: float = 150) -> Dict[str, Tuple[float, float]]:
    ...
    return {states[i]: (cx + r * math.cos(2 * math.pi * i / n - math.pi / 2),
...
class DFASimulator:
    def __init__(self):
        ...
        # UI
        iw, ih = 300, 40
        self.input_box = InputBox((SCREEN_WIDTH - iw) // 2, SCREEN_HEIGHT - 120, iw, ih, self.dfa.alphabet)
        bw, by = 100, SCREEN_HEIGHT - 60
        self.start_btn = Button(SCREEN_WIDTH // 2 - bw - 10, by, bw, 40, "Start")
        self.reset_btn = Button(SCREEN_WIDTH // 2 + 10, by, bw, 40, "Reset")
        self.mode_btn = Button(20, by, 120, 40, "Mode: Auto")
        self.step_btn = Button(SCREEN_WIDTH // 2 - 50, by + 50, 100, 35, "Step")
        self.mode = Mode.AUTO

        self.state_positions = calc_positions(list(self.dfa.states), SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 30)
    ...
    def step_sim(self):
        if self.mode == Mode.STEP and not self.animator.state == AnimState.MOVING:
            self.do_step()
    ...
    def do_step(self):
        ...
        if result is None:
            ...
            self.animator.start_transition(self.renderer.get_pos(frm), self.renderer.get_pos(to))

    def update(self, dt: float):
        self.animator.update(dt)
        if self.animator.state == AnimState.IDLE and self.sim.is_running and not self.animator.state == AnimState.PAUSE:
            if self.mode == Mode.AUTO:
                self.do_step()
        self.update_step_btn()
    ...
    def update_step_btn(self):
        if self.mode == Mode.STEP:
            self.step_btn.disabled = not (self.sim.is_running and self.animator.state != AnimState.MOVING)
    ...
    def render(self):
        ...
        if self.sim.is_running and self.sim.input_string:
            ...
            self.renderer.draw_symbol(self.sim.input_string[idx], idx, len(self.sim.input_string))
        ...
        self.renderer.draw_button(self.start_btn.rect, self.start_btn.label,
                                  self.start_btn.hovered(pygame.mouse.get_pos()), self.start_btn.disabled)
        self.renderer.draw_button(self.reset_btn.rect, self.reset_btn.label,
                                  self.reset_btn.hovered(pygame.mouse.get_pos()))
        self.renderer.draw_button(self.mode_btn.rect, self.mode_btn.label,
                                  self.mode_btn.hovered(pygame.mouse.get_pos()))
        if self.mode == Mode.STEP:
            self.renderer.draw_button(self.step_btn.rect, self.step_btn.label,
                                      self.step_btn.hovered(pygame.mouse.get_pos()), self.step_btn.disabled)
